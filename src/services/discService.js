...
exports.calculateDiscProfile = async (answers) => {
  try {
    ...
    if (
      ...
      primary.percentage >= secondary.percentage * 1.4
    ) {
      profile = primary.profile;
      analysisType = "dominant";
    } else if (
      Math.abs(primary.percentage - secondary.percentage) <= 8 &&
      primary.percentage >= 22
    ) {
      profile = `${primary.profile}${secondary.profile}`;
      analysisType = "balanced_hybrid";
    } else if (
      secondary.percentage >= primary.percentage * 0.65 &&
      primary.percentage >= 25
    ) {
      profile = `${primary.profile}${secondary.profile}`;
      analysisType = "moderate_hybrid";
    } else if (
      primary.percentage <= 30 &&
      secondary.percentage >= 20 &&
      third.percentage >= 18
    ) {
  ...
  switch (analysisType) {
    case "dominant":
      recommendations.push(
        `Seu perfil ${sortedScores[0].profile} é muito claro - use isso como sua principal força`
      );
      recommendations.push(
        `Considere desenvolver características ${sortedScores[1].profile} para maior versatilidade`
      );
      break;

    case "balanced_hybrid":
      recommendations.push(
        `Seu perfil ${sortedScores[0].profile}${sortedScores[1].profile} oferece grande flexibilidade`
      );
      recommendations.push(
        "Use esta versatilidade para se adaptar a diferentes situações"
      );
      break;

    case "moderate_hybrid":
      recommendations.push(
        `Você tem características ${sortedScores[0].profile} dominantes com influência ${sortedScores[1].profile}`
      );
      recommendations.push(
        "Esta combinação oferece equilíbrio entre força e adaptabilidade"
      );
      break;
    ...
    case "multi_faceted":
      ...
      recommendations.push(
        "Use esta adaptabilidade como uma grande vantagem profissional"
      );
      break;

    case "moderate":
      recommendations.push(
        `Seu perfil ${sortedScores[0].profile} é moderadamente forte`
      );
      recommendations.push(
        "Considere desenvolver características complementares"
      );
      break;

    default:
      recommendations.push(
        "Você tem um perfil equilibrado em todas as dimensões DISC"
      );
      recommendations.push(
        "Foque em desenvolver situações que maximizem seus pontos fortes"
      );
  }
...
function getWorkingStyle(profile, percentages) {
  ...
  switch (dominant) {
    case "D":
      style.approach =
        "Direto, focado em resultados e tomada de decisão rápida";
    ...
    case "I":
      ...
      style.challenges =
        "Pode ter dificuldade com tarefas detalhadas e solitárias";
      break;
    case "S":
    ...
    case "C":
      ...
      style.environment =
        "Ambientes organizados, com padrões claros e tempo para análise";
      style.motivation = "Precisão, qualidade e conhecimento técnico";
      style.challenges =
        "Pode ser perfeccionista e resistir a decisões rápidas";
      break;
  }
  ...
  switch (dominant) {
    case "D":
      ...
      style.challenges = [
        ...
        "Falta de consideração com sentimentos",
      ];
      style.teamManagement =
        "Prefere equipes eficientes e orientadas para metas";
      break;
    case "I":
      ...
      style.challenges = [
        "Pode negligenciar detalhes",
        "Dificuldade com confrontos",
        "Falta de follow-up",
      ];
...
function getCommunicationStyle(profile, percentages) {
  ...
  switch (dominant) {
    case "D":
      ...
      style.preferences =
        "Prefere comunicação rápida, direta e focada em resultados";
    ...
    case "I":
      ...
      style.preferences =
        "Prefere comunicação face a face, interativa e positiva";
    ...
    case "S":
      ...
      style.preferences =
        "Prefere comunicação calma, estruturada e não confrontativa";
    ...
    case "C":
      ...
      style.preferences =
        "Prefere comunicação escrita, detalhada e bem fundamentada";
...
function getStressReaction(profile, percentages) {
  ...
  switch (dominant) {
    case "D":
      ...
      reaction.management = [
        "Exercitar controle onde possível",
        "Focar em soluções",
        "Buscar desafios",
      ];
      break;
...
exports.getProfileCompatibility = (profile1, profile2) => {
  const compatibilityMatrix = {
    DD: {
      score: 60,
      description: "Podem competir, mas compartilham foco em resultados",
    },
    DI: { score: 75, description: "Complementam-se: direção e influência" },
    DS: {
      score: 70,
      description: "Boa complementaridade: decisão e estabilidade",
    },
...
exports.getDevelopmentSuggestions = (profile, percentages) => {
  ...
  const dominant = profile.charAt(0);
  const lowest = Object.entries(percentages).sort((a, b) => a[1] - b[1])[0][0];
